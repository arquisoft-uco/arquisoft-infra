...
first volume creation. `arquisoft_user` is the **app user** and is `NOSUPERUSER NOCREATEROLE
NOCREATEDB` (least privilege); the superuser is `postgres`. The Keycloak DB is **dedicated**
(`components/keycloak/keycloak-db`), not in the app PostgreSQL.

### Access model — admin/root + app credential per service (least privilege)
Every data service exposes **two** credentials: an **admin/root** one (console/administration) and
an **application** one (used by the backend) with least privilege. The identifier (user / access-key /
client-id) lives in `terraform.tfvars`; the secret is generated by the `secrets` module. **The
backend always uses the app credential, never admin.**
| Service | Admin/root | App (backend) | App secret |
|---------|-----------|---------------|-----------|
| PostgreSQL | `postgres` (superuser) | `arquisoft_user` (NOSUPERUSER) | `app_db_password` |
| RabbitMQ | `arquisoft_admin` (administrator) | `arquisoft_backend` (no admin tag) | `rabbitmq_app_password` |
...
| Keycloak | `admin` (console) | OIDC client `arquisoft-api` | `keycloak_client_secret` |
Redis ACL keeps `+info` (Spring Boot's health check needs `INFO`, which is in `@dangerous`).
...
| `components/rabbitmq/config/definitions.json.template` | `…/definitions.json` | `RABBITMQ_USER/PASSWORD` (admin), `RABBITMQ_APP_USER/PASSWORD` (app) |
